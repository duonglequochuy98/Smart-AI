...
export const BookingView: React.FC<BookingViewProps> = ({ onBack, onAddNotification }) => {
  ...
  const WEBHOOK_URL = "https://hook.eu1.make.com/9bn1blb1t9whz28xjag60toaqbk59fu7"
  ...
  const handleComplete = async () => {
    setIsProcessing(true);
    const datePart = formData.dateString.split(',')[1] || formData.dateString;
    
    try {
      // Gửi dữ liệu đến Webhook Make.com
    ...
    setTimeout(() => {
      ...
      onAddNotification({
        ...
        summary: `Mã cuộc hẹn ${smartCode} của ông/bà ${formData.name} đã được xác nhận vào lúc ${formData.time} ngày ${datePart}.`,
        date: newsDate,
        category: 'Thông báo',
        isRead: false,
        isBooking: true,
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar flex flex-col items-center p-6 text-center">
          <div className="pt-4 space-y-3 shrink-0">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[9px] text-yellow-100/40 font-bold uppercase tracking-[0.2em]">Hệ thống đã tự động tải phiếu hẹn và gửi email xác nhận</p>
            </div>
          </div>
             ...
             <div className="space-y-4">
               ...
               <div className="space-y-3">
                 ...
                 <div className="relative group pt-2">
                   ...
                   <input type="email" placeholder="Email nhận xác nhận" value={formData.email} className="w-full h-14 bg-white border border-slate-200 rounded-2xl pl-12 pr-4 text-sm font-bold focus:border-red-500 outline-none" onChange={(e) => setFormData({...formData, email: e.target.value})} />
                 </div>
               </div>
             </div>
