...
import { ArrowLeft, Bell, Calendar, ChevronRight, Megaphone, Info, Search, CalendarCheck, X, ShieldCheck, Barcode, Volume2 } from 'lucide-react';
...
export const NotificationView: React.FC<NotificationViewProps> = ({ onBack, notifications, setNotifications }) => {
  ...
  const speak = (title: string, summary: string, id: number) => {
    ...
    window.speechSynthesis.cancel();
    const textToRead = `${title}. Nội dung tóm tắt: ${summary}`;
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-24 space-y-4 no-scrollbar">
        {filteredNews.length === 0 ? (
          ...
          filteredNews.map((news) => (
            <div 
              key={news.id}
              className={`w-full group p-4 rounded-[24px] border transition-all active:scale-[0.99] relative overflow-hidden flex gap-4 ${
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        speak(news.title, news.summary, news.id);
                      }}
                      className={`p-1.5 rounded-full transition-all ${isSpeakingId === news.id ? 'bg-red-50 text-red-600 animate-pulse' : 'hover:bg-slate-50 text-slate-400'}`}
                    >
                      <Volume2 size={16} />
                    </button>
