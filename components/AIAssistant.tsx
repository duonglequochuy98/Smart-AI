...
const UI_TEXT = {
  vi: {
    ...
    audioOn: 'Bật giọng nói',
    audioOff: 'Tắt giọng nói'
  },
  en: {
    title: 'Assistant',
...
export const AIAssistant: React.FC<AIAssistantProps> = ({ onBack }) => {
  ...
  const speak = (text: string, index: number) => {
    ...
    // Clean text for better reading (remove markdown-like characters)
    const cleanText = text.replace(/[*#]/g, '').replace(/https?:\/\/\S+/g, 'liên kết trang web');
    
    const utterance = new SpeechSynthesisUtterance(cleanText);
    utterance.lang = lang === 'vi' ? 'vi-VN' : 'en-US';
    utterance.rate = 1.0; // Moderate speed for elderly
    utterance.pitch = 1.0;
    ...
    // Find a Vietnamese voice if language is VI
    if (lang === 'vi') {
  ...
  const handleSend = async (customInput?: string) => {
    ...
    try {
      ...
      const aiReply = reply || UI_TEXT[lang].error;
      const newMessages: Message[] = [...messages, userMsg, { role: 'model', text: aiReply }];
      setMessages(newMessages);
      
      // Auto-speak if enabled
      if (isSpeechEnabled) {
        setTimeout(() => {
          speak(aiReply, newMessages.length - 1);
        }, 500);
      }
    } catch (error) {
      ...
      <div className="p-4 border-b flex items-center justify-between bg-red-600 text-white shadow-md z-10">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className={`p-2 rounded-xl transition-all ${isSpeechEnabled ? 'bg-white text-red-600' : 'bg-red-700/50 text-white/60'}`}
            title={isSpeechEnabled ? UI_TEXT[lang].audioOff : UI_TEXT[lang].audioOn}
          >
              ...
              <div className="relative group">
                ...
                {m.role === 'model' && (
                  <div className="absolute -bottom-6 left-2 flex items-center gap-3">
                    ...
                    <button 
                      onClick={() => isSpeaking === i ? window.speechSynthesis.cancel() : speak(m.text, i)}
                      className={`p-1 transition-colors ${isSpeaking === i ? 'text-red-600 animate-pulse' : 'text-slate-400 hover:text-red-600'}`}
                    >
                      <Volume2 size={12} />
                    </button>
                  </div>
